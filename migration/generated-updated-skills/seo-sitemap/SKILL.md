...
## HighLevel-hosted sitemap notes

Add a HighLevel mode when the audited site is hosted on HighLevel Sites, Funnels, Websites, Stores, or Blogs.

In that mode:
- Validate that published HighLevel pages are included where expected.
- Check canonical alignment between sitemap URLs and the live page URL structure.
- Flag funnel steps, thank-you pages, drafts, redirects, and noindex pages that should not appear in the sitemap.
- Review blog and website page coverage separately when HighLevel Blogs are part of the stack.
- Keep the underlying XML validation rules unchanged.

When the user needs implementation help, use the shared `highlevel-api` skill for live page, blog, or location-aware lookups.
