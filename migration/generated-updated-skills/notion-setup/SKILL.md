...
| Batch | `rich_text` | |
| Post Date | `date` | |
| Script URL | `url` | Primary long-form script or source asset URL |
| Hand Raiser URL | `url` | Direct file or asset URL |
| Hand Raiser Page | `url` | Public delivery page URL |
| HighLevel Funnel URL | `url` | Funnel or site page used for delivery |
| HighLevel Form URL | `url` | Lead capture form or survey URL |
| HighLevel Calendar URL | `url` | Booking URL when the CTA is appointment-based |
| HighLevel Resource URL | `url` | Asset or resource URL inside HighLevel |
| Google Doc URL (Legacy) | `url` | Optional only if Docs is still used |
...
> Notion pipeline is ready! Your Script Library database is set up with all core properties, including HighLevel companion URL fields.
>
> When you run `/content-master` or `/content-scripting`, scripts can flow into Notion while HighLevel owns capture, delivery, workflows, calendars, and CRM activity.
>
> Use `/content-pipeline` to manage your production pipeline — update statuses, log metrics, and keep HighLevel asset links tied to each record.

---

## HighLevel companion fields

Keep Notion as the system of record for scripts. Add HighLevel URL fields so downstream skills can attach capture and delivery assets without falling back to Google Docs.

Recommended fields include:
- `Script URL`
- `Hand Raiser URL`
- `Hand Raiser Page`
- `HighLevel Funnel URL`
- `HighLevel Form URL`
- `HighLevel Calendar URL`
- `HighLevel Resource URL`
- `Google Doc URL (Legacy)` only if the user still wants it
