...
After scripts are saved to a markdown file, push them into the current stack:

1. **Check Notion** — If `~/.config/notion-content/config.json` exists:
   - read the config to get `script_library_db_id`
   - parse each `## SF #{N} - {Title}` from the markdown
   - create or update a Script Library entry for each script with Script Title, Script Number, Status, Series, Topic, Hook Framework A/B/C, Batch, and page content containing the script body or the local file path
   - where possible, write back any generated HighLevel URLs such as Funnel, Form, Calendar, Resource, Blog Post, or Social Planner links

2. **Check HighLevel** — If the shared `highlevel-api` skill is available and the user has supplied auth plus context:
   - create or update the HighLevel capture path for the script CTA, usually a funnel, site page, form, survey, calendar, workflow, or blog post
   - map comment-keyword CTAs to HighLevel Instagram Comment Automation and DM workflows
   - store tags, custom fields, attribution values, and opportunity routing in HighLevel
   - wire email, SMS, reminder, and nurture follow-up inside HighLevel workflows

3. **Report:**
   > Scripts pushed to:
   > - Notion: {N} entries created or updated in Script Library
   > - HighLevel: {N} CTA or delivery paths created or updated when configured
   >
   > Next: use `/content-pipeline` to manage filming status, or continue into hand raisers and publishing.

If neither integration is configured, skip silently and keep the local markdown artifact.
...
## Notion + HighLevel production pipeline

Keep Notion as the content database. Use HighLevel as the default capture, delivery, nurture, and CRM execution layer.

### Pipeline default
- scripts and research live in Notion
- lead magnets, funnels, forms, surveys, calendars, workflows, and CRM activity live in HighLevel
- comment-keyword CTAs use HighLevel Instagram comment automation and DM workflows
- follow-up uses HighLevel email, SMS, tags, custom fields, and opportunity routing

### Shared skill usage
If HighLevel auth and context are available, use the shared `highlevel-api` skill to create or update contacts, opportunities, blog posts, calendars, forms, workflows, Social Planner assets, or delivery URLs.
