---
name: client-brief
description: Generate a 2-page interactive client proposal (brief + work plan) with SVG diagrams, accordion deliverables, architecture maps, ROI charts, and HighLevel Sites or Funnels deployment, with HTML export optional.
...
**Step 2 (optional):** If you want a standalone export outside HighLevel, keep the generated HTML files. HighLevel is the default deployment target, so no extra hosting setup is required for the normal workflow.

That's it. No extra hosting account is required for the default path.
...
**Step 5:** Claude publishes the proposal into HighLevel Sites or Funnels by default, wires the CTA to a HighLevel form or calendar when needed, and can set up pipeline opportunity creation plus follow-up workflows. HTML export remains optional if you want to host it elsewhere.
...
**Do I need Vercel?**
No. HighLevel Sites or Funnels are now the default delivery path. The HTML files still work locally and can be hosted elsewhere if you want an export-only version.
...
## HighLevel-first proposal delivery

HighLevel Sites or Funnels are the default deployment path for this skill. HTML export remains optional.

### Default implementation
- publish the proposal as a HighLevel site page or funnel step
- attach a HighLevel form or calendar when the client needs to book, apply, or request next steps
- create or update the opportunity in the correct pipeline on submission or booking
- trigger HighLevel workflows for email, SMS, reminders, and follow-up

### Output rule
When the user asks for a proposal build, return the HighLevel deployment plan first. Treat Vercel or other hosting as optional export paths only.
